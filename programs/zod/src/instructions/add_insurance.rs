...
pub struct ZodAddInsurance<'info> {
  #[account(mut)]
...
pub fn process(cx: Context<ZodAddInsurance>, amount: u64) -> ProgramResult {
  msg!("Instruction: ZodAddInsurance");
  ...
  assert!(col_index == 0);
  assert!(zo_program_state.vaults[col_index] == zo_vault.key());
